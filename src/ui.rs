...
pub fn dir_listing(path: &str, trim: &str) -> Box<Future<Item=HttpResponse, Error=Error>> {
	...
	let mut html = [HEAD, "<title>Directory listing of ", &path[trim.len()..], "</title><h1 class=ok>", FOLDERSVG, "Directory listing of ", &path[trim.len()..], "</h1><span style='left: 32px;position: relative;bottom: 6px;'>Name</span><span style='left: 256px;position: relative;bottom: 6px;'>Size</span></br><a href='..'>", BACKSVG, "Back</a></br>"].concat();
	for fpath in f {
		...
		html = [&html, "<a href='", &encode_attribute(fstr.to_string_lossy()[trim.len()..].borrow()), "'>", icon, &encode_minimal(name.borrow()), "<span style='left: 356px;position: absolute;padding-top: 12px;'>", &sizestr, "</span></a></br>"].concat()
	}

	html = [html, "<span class='bottom right'>Powered by KatWebX</span>".to_string()].concat();
