...
pub fn dir_listing(path: &str, trim: &str) -> Box<Future<Item=HttpResponse, Error=Error>> {
	...
	let mut html = [HEAD, "<h1>Directory listing of ", &path[trim.len()..], "</h1><a href='..'>", BACKSVG, "Back</a></br>"].concat();
	for fpath in f {
	...
	html = [html, "<span class=bottom style='right:32px'>Powered by KatWebX</span>".to_string()].concat();
...
pub fn http_error(status: StatusCode, header: &str, body: &str) -> Box<Future<Item=HttpResponse, Error=Error>> {
	return result(Ok(
		HttpResponse::Ok()
			.status(status)
			.content_type("text/html; charset=utf-8")
			.body([HEAD, "<h1 style='color: #b42020'>", ERRSVG, &encode_minimal(header), "</h1><p>", &encode_minimal(body), "</p><span class=bottom>Powered by KatWebX</span>"].concat())))
			.responder();
}
