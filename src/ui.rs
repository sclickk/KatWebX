...
pub fn dir_listing(path: &str, trim: &str) -> Box<Future<Item=HttpResponse, Error=Error>> {
	...
	let mut html = [HEAD, "<title>Directory listing of ", &path[trim.len()..], "</title><h1 style='fill:#20b48c'>", FOLDERSVG, "Directory listing of ", &path[trim.len()..], "</h1><a href='..'>", BACKSVG, "Back</a></br>"].concat();
	for fpath in f {
		...
		html = [&html, "<a href='", &encode_attribute(fstr.to_string_lossy()[trim.len()..].borrow()), "'>", icon, &encode_minimal(name.borrow()), "</a></br>"].concat()
	}
...
pub fn http_error(status: StatusCode, header: &str, body: &str) -> Box<Future<Item=HttpResponse, Error=Error>> {
	return result(Ok(
		HttpResponse::Ok()
			.status(status)
			.content_type("text/html; charset=utf-8")
			.body([HEAD, "<title>", header, "</title><h1 style='color: #b42020'>", ERRSVG, &encode_minimal(header), "</h1><p>", &encode_minimal(body), "</p><span class=bottom>Powered by KatWebX</span>"].concat())))
			.responder();
}
