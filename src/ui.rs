...
pub fn dir_listing(path: &str, trim: &str) -> Box<Future<Item=HttpResponse, Error=Error>> {
	...
	let mut html = [HEAD, "<title>Directory listing of ", &path[trim.len()..], "</title><h1 class=ok>", FOLDERSVG, "Directory listing of ", &path[trim.len()..], "</h1><span style='left: 32px;position: relative;bottom: 6px;'>Name</span><span class=righthov>Size</span></br><a href='..'>", BACKSVG, "Back</a></br>"].concat();
	for fpath in f {
